...
- `flark_v2_tasks_tables_overlays.png`: task checkboxes, table grid
  rendering, link/image/action overlay chips, and task/table overlay labels.
- `flark_v2_compact_mixed.png`: compact preview layout with wrapping,
  overlays, quote rail, code block, and task rows in a narrow viewport.

## Update Command

```bash
flutter test --update-goldens test/v2/flutter/flark_v2_visual_golden_test.dart
```
