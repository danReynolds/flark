...
| --- | --- | --- |
| `markdown/parse/flark_native_comrak_parse_backend.dart` | mapping-layer synthesis | **Phase 2 (in progress):** link/image scanning deleted — markup arrives as AST-derived per-token ranges; the flanking import is gone. Fence synthesis (fenced-code scanner import) remains until Phase 4 |
...
| 2 | Bridge protocol v2: marker sub-ranges for links/images/autolinks, multi-line ref-def spans, every structural fact as ranges; new thin Dart decoder cut over under conformance/parity/goldens; carry the sync-parse ceiling in the bridge call (deletes the backend threshold shim); fix the abutting-closers gap (`**foo *bar***`): root cause was NOT the bridge (it emits all four marker ranges — cargo-pinned) but the Dart partial-strong-intent heuristic misreading parsed adjacency as mid-typing; it now defers to the parser (adjacency counts only when the neighbor char is unclaimed by any parsed marker range), and the judge pin flipped to committed | `_nativeInlineHiddenRanges`, `_referenceDefinitionRanges`, the mapper's scanner imports (~1,400 of 1,853 lines) | pending |
| 3 | One shared headless segmentation module (`render_plan/flark_inline_segmentation.dart`) consumed by BOTH surfaces — the preview's span construction and the live editor's `_blockTextSegments` cut overlapping runs through the same implementation; differential preview-vs-editor oracle (character-multiset comparison — placeholder/order-immune, unforgiving of duplication/omission/marker leaks) over a 15-case inline corpus | preview's bespoke boundary walk and the live surface's duplicate of it | shipped |
