...
**Date**: 2026-03-16  
**Related docs**:
- `docs/architecture/flark/flark_editor_how_it_works.md`
- `docs/architecture/flark/flark_editor_markdown_support_matrix.md`
- `docs/architecture/rfc/rfc_016_flark_markdown_command_layer.md`

Implementation update (2026-03-16):

- `FlarkMarkdownView` now exists in package API and uses flark
...
- Feed excerpt previews now render markdown via `PostReadMarkdown` for both
  non-clamped and clamped paths (clamped path uses flark clip + read-more overlay).

Rollout decision status (current):

- **Detail screens**: use flark read-only renderer now.
- **Feed/excerpt screens**: cut over for markdown content to flark read rendering.
...
1. **Edit mode**: Flark editor path (`FlarkEditor` + native `comrak` parse + flark projection/rendering).
...
1. Replacing flark editing internals.
...
**Pros**
- Reuses existing flark render path quickly.
...
Proceed with **Option C**: build a dedicated read-only flark renderer surface
...
- Reuse flark syntax pipeline and snapshot normalization.
- Reuse flark block/background/inline rendering primitives.
...
2. Decide final rollout scope:
   - flark view everywhere, or
   - flark view for detail + existing path for large feed cards.
...
3. For large feeds, do we prefer: -> feeds will be small
   - flark view only in detail surfaces, or
   - flark view everywhere with aggressive caching/virtualization?
