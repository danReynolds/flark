...
- `presentation/flark_markdown_view.dart`
  - read-only package surface for flark-backed rendering in app post detail screens.

## 3) Parsing Profiles + Contracts

Profiles are defined by the public `MarkdownSyntaxProfile` enum in
`lib/widgets/flark/engine/syntax_engine.dart`:
...
- `docs/architecture/rfc/flark_editor_command_interface.md`
