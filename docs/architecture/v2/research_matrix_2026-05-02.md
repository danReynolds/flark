# Sovereign v2 Research Matrix

Status date: 2026-05-02

## Scope

This research pass informs the v2 rewrite direction for Sovereign as a
best-in-class Dart/Flutter markdown editing and previewing library. The current
Sovereign package remains the reference implementation, regression oracle, and
compatibility baseline while v2 is designed as a cleaner architecture.

## Summary Thesis

Sovereign v2 should be a source-first markdown editor engine with a headless
...
| Area | Source | Relevant finding | Decision for Sovereign v2 |
| --- | --- | --- | --- |
...
| Flutter renderer ecosystem | `flutter_markdown_plus`, https://pub.dev/packages/flutter_markdown_plus | The maintained continuation of `flutter_markdown` is a renderer, built on `markdown`, and defaults to GitHub Flavored Markdown. It explicitly does not support inline HTML. | The ecosystem already has renderers. Sovereign should differentiate by offering live source editing plus preview parity, not just Markdown-to-widget rendering. |
...
| Markdown editor product shape | Milkdown, https://milkdown.dev/core | Milkdown is a plugin-driven, headless WYSIWYG Markdown editor framework built on ProseMirror, Y.js, and Remark. | Sovereign should be plugin-driven and headless, but its core value should be source-faithful Markdown editing in Flutter rather than DOM/ProseMirror semantics. |
...
| Rich Flutter alternatives | `flutter_quill`, https://pub.dev/packages/flutter_quill | Flutter Quill stores content as Quill Delta JSON and recommends saving Delta instead of HTML/Markdown for fidelity. | This validates a key differentiation: Sovereign should be the library to choose when Markdown source itself is the durable content format. |
