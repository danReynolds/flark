...
- New IME input suite drives real composing-region sequences through the
  ...
  `legacy/docs/v2_v3/testing/ime_device_protocol.md`. New widget-level
  `LiveRenderSequence`
...
- Inline styled runs (code/strong/emphasis/strikethrough) can be re-entered
  ...
  in `legacy/docs/v2_v3/doc/architecture/live_edit_intent_pipeline.md`.
...
- Platform-echo handling is classified, not open-coded: each editing surface
  ...
  `legacy/docs/v2_v3/doc/architecture/live_edit_intent_pipeline.md` and pinned
  by tests; the
