...
import 'flark_default_parse_backend.dart';
...
final class FlarkFlutterController extends ChangeNotifier {
  ...
  FlarkParseScheduler _ensureScheduler() {
    return _parseScheduler ??= FlarkParseScheduler(
      controller: this,
      backend: _parseBackend ?? flarkDefaultParseBackend(),
  ...
  static List<(int, int)> _sortedHiddenBounds(
    ...
    final sorted = [
      for (final hidden in hiddenRanges) (hidden.range.start, hidden.range.end),
  ...
  void _debugConfirmPredictionAdoption(FlarkProjection authoritative) {
    ...
    for (final hidden in prediction.projection.hiddenRanges) {
      ...
      if (!authoritativeHidden.contains((
        hidden.range.start,
        hidden.range.end,
      ))) {
