...
extension _FenceNavigationPolicyOps on SovereignController {
  TextEditingValue _maybeExitFencedCodeOnArrowDown(
    ...
    if (!_shouldExitFenceOnArrowDown(
      text: arrow.text,
      context: context,
    ...
    if (closeLine != null && closeLine > context.openLine) {
      final closeLineStart = _lineIndex.offsetAtLine(closeLine);
      final trimStart = _trailingBlankTrimStart(
        arrow.text,
        context.openLine,
        closeLine,
      );
      if (trimStart < closeLineStart) {
        ...
        var exitOffset = (context.block.endOffset - deletedLen).clamp(
          0,
          exitText.length,
        );
    ...
    final exitOffset = context.block.endOffset.clamp(0, arrow.text.length);
    return newValue.copyWith(
      selection: TextSelection.collapsed(offset: exitOffset),
      composing: TextRange.empty,
    );
  ...
  TextEditingValue _maybeExitFencedCodeOnArrowUp(
    TextEditingValue oldValue,
    TextEditingValue newValue,
  ) {
    final arrow = _VerticalArrowEditContext.detect(
    ...
    final exitOffset = context.block.startOffset.clamp(0, arrow.text.length);
    return newValue.copyWith(
      selection: TextSelection.collapsed(offset: exitOffset),
      composing: TextRange.empty,
    );
