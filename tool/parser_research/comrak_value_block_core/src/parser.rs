...
use generated_scanner_gate::{
    AtxLineCuts, CursorScanError, FUSED_ATX_MAX_SOURCE_ACCESSES_PER_POLL, FusedAtxDonorMatch,
    FusedAtxLineScanError, FusedAtxLineScanResult, FusedAtxLineScanner, FusedAtxLineSource,
};
...
impl DirectRecipeAdmissionReceipt {
    ...
    const fn line_length_independent_part(self) -> (usize, usize, usize, usize, usize, usize) {
        (
            ...
            self.requested_retired_slots,
            self.requested_old_source_slots,
            self.requested_body_slots + self.requested_last_use_slots,
        )
    }
}
...
impl DirectHooks {
    ...
    fn seal_recipe(&mut self) -> Result<(), ParseError> {
        ...
        let retired = self
            ...
            .filter_map(|intent| match intent {
                DirectIntent::Close { node, .. } => Some(*node),
                _ => None,
            })
            .collect::<HashSet<_>>();
        if self.body.iter().any(|intent| {
            intent
                .required_old_owner()
                .is_some_and(|owner| retired.contains(&owner))
                || intent
                    .separate_logical_target()
                    .is_some_and(|target| retired.contains(&target))
        }) {
            return Err(ParseError::Invariant(
    ...
    fn retired_top_last_use(&self) -> Result<Option<usize>, ParseError> {
        let Some(DirectIntent::Close { node, .. }) = self.retired.front() else {
            if self.retired.is_empty() {
                return Ok(None);
            }
            return Err(ParseError::Invariant("retired recipe contains only closes"));
        };
    ...
    fn pop_old_boundary_intent(&mut self) -> Result<Option<DirectIntent>, ParseError> {
        if let Some(last_use) = self.retired_top_last_use()? {
            if last_use < self.old_source_index {
                return Ok(self.retired.pop_front());
            }
...
const fn direct_grammar_kind_can_contain(
    ...
    match parent {
        ...
        | DirectGrammarKind::FencedCode { .. } => false,
    }
}
...
impl ValueBlockParser {
    ...
    fn settle_open_new_transition(
        ...
        if open.current_before_open == self.current {
    ...
    fn enter_open_new(
        ...
        let current_before_open = self.current;
    ...
    fn handle_blockquote(
        ...
        *container = self.add_child(*container, BlockKind::BlockQuote, start + 1)?;
        if let Some(direct) = &mut self.direct {
    ...
    fn resolve_reference_link_definitions(&mut self, node: NodeId) -> Result<bool, ParseError> {
        let logical = self.tree.node(node).content.logical.clone();
        let definitions = reference_definitions(&logical)?;
        ...
        for definition in definitions {
            self.references.push(ReferenceOccurrence {
                ...
                title: definition.resolved.title,
                origins: self
                    .tree
                    .node(node)
                    .content
                    .origins_for_range(definition.source),
            });
        }
        if consumed > 0 {
            self.tree.node_mut(node).content.drain_prefix(consumed);
            self.tree.events.push(BlockEvent::DrainContentPrefix {
                node,
                bytes: u32::try_from(consumed).expect("reference prefix below u32"),
            });
        }
        Ok(!self.tree.node(node).content.is_blank())
    }
