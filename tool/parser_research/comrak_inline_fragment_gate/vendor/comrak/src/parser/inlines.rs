...
pub(crate) enum InlineAnnotationKind {
    Marker,
    Replacement(String),
    LinkSpan {
        image: bool,
...
pub(crate) struct InlineReferenceResolution {
    pub symbol_id: u64,
    pub presence_generation: u64,
    pub reference: Option<ResolvedReference>,
}

pub(crate) trait InlineReferenceResolver {
    fn resolve_inline_reference(
        &self,
        normalized: &str,
        original: &str,
    ) -> InlineReferenceResolution;
}
...
pub struct Subject<'a: 'd, 'r, 'o, 'd, 'c, 'p> {
    ...
    inline_reference_resolver: Option<&'o dyn InlineReferenceResolver>,
}
...
impl<'a, 'r, 'o, 'd, 'c, 'p> Subject<'a, 'r, 'o, 'd, 'c, 'p> {
    pub fn new(
        ...
        let mut s = Subject {
            ...
            annotations: Vec::new(),
            inline_reference_resolver: None,
        };
        for &b in b"\n\r_*\"`\\&<[]!$" {
    ...
    pub(crate) fn enable_annotations(&mut self) {
        self.collect_annotations = true;
    }

    pub(crate) fn take_annotations(&mut self) -> Vec<InlineAnnotation> {
        mem::take(&mut self.annotations)
    }

    pub(crate) fn set_inline_reference_resolver(
        &mut self,
        resolver: &'o dyn InlineReferenceResolver,
    ) {
        self.inline_reference_resolver = Some(resolver);
    }

    fn annotate_marker(&mut self, start: usize, end: usize) {
        if self.collect_annotations && start < end {
            self.annotations.push(InlineAnnotation {
    ...
    fn annotate_replacement(&mut self, start: usize, end: usize, replacement: String) {
        if self.collect_annotations && start < end {
            self.annotations.push(InlineAnnotation {
                kind: InlineAnnotationKind::Replacement(replacement),
                start,
                end,
            });
        }
    ...
    fn handle_newline(&mut self) -> Node<'a> {
        ...
        {
            self.annotate_marker(nlpos - 2, nlpos);
    ...
    fn handle_backticks(&mut self, parent_line_offsets: &[usize]) -> Node<'a> {
        ...
        let replacement = buf.to_string();
        let code = NodeCode {
            num_backticks: openticks,
            literal: replacement.clone(),
        };
        ...
        self.annotate_marker(startpos, startpos + openticks);
        self.annotate_marker(endpos - openticks, endpos);
        self.annotate_replacement(startpos + openticks, endpos - openticks, replacement);
    ...
    fn handle_backslash(&mut self, parent_line_offsets: &[usize]) -> Node<'a> {
        ...
        if self.peek_byte().is_some_and(ispunct) {
            self.scanner.pos += 1;
            self.annotate_marker(startpos, startpos + 1);
    ...
    fn handle_pointy_brace(&mut self, parent_line_offsets: &[usize]) -> Node<'a> {
        ...
        if let Some(matchlen) = scanners::autolink_uri(&self.input[self.scanner.pos..]) {
            ...
            let marker_start = self.scanner.pos - 1 - matchlen;
            let marker_end = self.scanner.pos - 1;
            ...
            self.annotate_marker(marker_start, marker_start + 1);
            self.annotate_marker(marker_end, marker_end + 1);
            return inl;
        }
    ...
    fn handle_delim(&mut self, b: u8) -> Node<'a> {
        ...
        {
            self.push_delimiter(b, can_open, can_close, numdelims, inl);
        }
    ...
    fn push_delimiter(
        ...
        let length = inl.data().value.text().unwrap().len();
        let d = self.delimiter_arena.alloc(Delimiter {
            ...
            length,
            source_start: Cell::new(self.scanner.pos - source_length),
            source_length: Cell::new(source_length),
            delim_byte: c,
            can_open,
            can_close,
        });
    ...
    fn insert_emph(
        ...
        let opener_source_start = opener.source_start.get();
        let opener_source_length = opener.source_length.get();
        let closer_source_start = closer.source_start.get();
        let closer_source_length = closer.source_length.get();
        if opener_source_length == opener_num_bytes + use_delims
            && closer_source_length == closer_num_bytes + use_delims
        {
            let opening_start = opener_source_start + opener_source_length - use_delims;
            let closing_start = closer_source_start;
            self.annotate_marker(opening_start, opening_start + use_delims);
            self.annotate_marker(closing_start, closing_start + use_delims);
        }
        let opener_source_use = use_delims.min(opener_source_length);
        opener
            .source_length
            .set(opener_source_length - opener_source_use);
        let closer_source_use = use_delims.min(closer_source_length);
        closer
            .source_start
            .set(closer_source_start + closer_source_use);
        closer
            .source_length
            .set(closer_source_length - closer_source_use);
    ...
    fn push_bracket(&mut self, image: bool, inl_text: Node<'a>) {
        ...
        self.brackets.push(Bracket {
            inl_text,
            position: self.scanner.pos,
            source_start: self.scanner.pos - if image { 2 } else { 1 },
            image,
            bracket_after: false,
        });
        if !image {
    ...
    fn handle_close_bracket(&mut self, parent_line_offsets: &[usize]) -> Option<Node<'a>> {
        ...
        let mut inline_resolution = None;
        if reff.is_none() {
            if found_label
                && !normalized_lab.is_empty()
                && let Some(resolver) = self.inline_reference_resolver
            {
                let resolution = resolver.resolve_inline_reference(&normalized_lab, &lab);
                let resolved = resolution.reference.is_some();
                inline_resolution = Some((
                    resolution.symbol_id,
                    resolution.presence_generation,
                    resolved,
                ));
                reff = resolution.reference.map(Cow::Owned);
            } else if let Some(callback) = &self.options.parse.broken_link_callback {
                reff = callback
        ...
        if self.collect_annotations
            && found_label
            && !normalized_lab.is_empty()
            && let Some((symbol_id, presence_generation, resolved)) = inline_resolution
        {
            self.annotations.push(InlineAnnotation {
                kind: InlineAnnotationKind::ReferenceQuery {
                    normalized_label: normalized_lab.clone(),
                    symbol_id,
                    presence_generation,
                    resolved,
                    image: is_image,
                },
                start: last.source_start,
        ...
        if let Some(reff) = reff {
            ...
            self.close_bracket_match(
                ...
                !(!is_image
                    && self.scanner.pos == initial_pos
                    && self.input[last.position..initial_pos - 1].starts_with('^')),
                parent_line_offsets,
            );
            return None;
        }
    ...
    fn close_bracket_match(
        ...
        semantic_end_pos: usize,
        tail_start_pos: usize,
        ...
        if self.collect_annotations {
            self.annotations.push(InlineAnnotation {
                kind: InlineAnnotationKind::LinkSpan { image: is_image },
                start: last.source_start,
                end: semantic_end_pos,
            });
        }

        if emit_projection {
            self.annotate_marker(last.source_start, last.position);
            self.annotate_marker(tail_start_pos, semantic_end_pos);
        }
...
pub struct Delimiter<'a: 'd, 'd> {
    ...
    position: usize,
    length: usize,
    source_start: Cell<usize>,
    source_length: Cell<usize>,
    delim_byte: u8,
    can_open: bool,
    can_close: bool,
