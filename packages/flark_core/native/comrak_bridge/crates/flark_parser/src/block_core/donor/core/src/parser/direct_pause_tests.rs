...
#[test]
fn gfm_profile_survives_both_durable_restart_codecs() {
    let mut parser = DirectValueBlockParser::new(SyntaxProfile::Gfm).expect("GFM parser");
    parser
        .acknowledge_command()
        .expect("acknowledge document open");
    ...
    let durable = pause
        ...
        .expect("durable GFM capture");
    let resumed = DirectValueBlockParser::resume_durable_line_boundary_checkpoint(
        durable.header(),
        durable.frame_records(),
        cursor,
    )
    .expect("resume durable GFM parser");
    assert_eq!(resumed.parser.profile, SyntaxProfile::Gfm);

    let grammar = parser
        .capture_durable_grammar_line_boundary_checkpoint()
        .expect("durable GFM grammar capture");
    let (decoded, _) = DirectValueBlockParser::decode_durable_grammar_restart_parts(
        grammar.header(),
        grammar.frame_records(),
    )
    .expect("decode durable GFM grammar");
    assert_eq!(decoded.profile, SyntaxProfile::Gfm);
}
