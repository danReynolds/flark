...
impl<Identity: Copy + Eq> FusedAtxLineScanner<Identity> {
    ...
    /// Logical lookahead reservation already absorbed by the fused scanner.
    ///
    /// A persistent production source grant counts only unread physical bytes,
    /// whereas the generated opener conservatively reserves its replay-free
    /// lookahead on every poll. The outer adapter may add exactly this value to
    /// the reported logical budget; actual first reads still pass through the
    /// source's unchanged physical budget.
    #[must_use]
    pub const fn logical_access_budget_slack(&self) -> usize {
        if matches!(self.phase, FusedAtxPhase::Opener) {
            CURSOR_ATX_MAX_LOOKAHEAD_SLACK
        } else {
            0
        }
    }
