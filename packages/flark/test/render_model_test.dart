...
void main() {
  ...
  test(
    'a paragraph with emphasis decodes into hidden delimiters and content',
    () {
  ...
  test(
    'a run-less block yields an empty run range, not the next block\'s runs',
    () {
      final m = backend.parse('para\n\n---\n\n*x*');
      final rule = m.blocks.firstWhere(
        (b) => b.kind == BlockKind.thematicBreak,
      );
      expect(m.runsOfBlock(rule.index), isEmpty);
      final last = m.blocks.last;
      expect(m.runsOfBlock(last.index).map((r) => r.kind), [
        RunKind.emph,
        RunKind.text,
      ]);
    },
  ...
  test('an unaligned byte view is accepted', () {
    ...
    final padded = Uint8List(m.bytes.length + 2)
      ..setRange(2, m.bytes.length + 2, m.bytes);
    ...
    for (final file in ['common_mark_tests.json', 'gfm_tests.json']) {
      ...
      for (final c in list) {
        ...
        for (final r in m.runs) {
          expect(
            r.startUtf16 <= r.contentStartUtf16 &&
                r.contentStartUtf16 <= r.contentEndUtf16 &&
                r.contentEndUtf16 <= r.endUtf16,
            isTrue,
            reason: '$file #${c['example']} run ${r.index}',
          );
  ...
  test('malformed host UTF-16 is rejected before encoding', () {
    expect(
      () => backend.parse('x\uD800y'),
      throwsA(
  ...
  test('an extraction deviation fails closed at the transport', () {
    final source = '${'| a ' * 17}|\n${'|---' * 16}|-:|\n';
    expect(
