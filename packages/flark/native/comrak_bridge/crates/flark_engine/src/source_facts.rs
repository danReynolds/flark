...
use crate::measured_sequence::{
    begin_measured_sequence_seal, locate_measured_sequence_leaf_with_prefix,
    retain_committed_measured_sequence_root, retain_committed_measured_sequence_root_with_measure,
    splice_measured_sequence_atomic, splice_measured_sequence_build_root_atomic,
    validate_measured_sequence_build_owner, validate_measured_sequence_node,
    CommittedMeasuredSequenceRoot, CommittedMeasuredSequenceRootReleaseFailure,
    MeasuredSequenceBuildRoot, MeasuredSequenceSeal, ResumableMeasuredSequenceBuilder,
    ResumableSequenceProgress, SequenceInspectionReceipt, SequenceMeasure, SequenceMutationReceipt,
    SequenceSpec, SequenceSpecInspection, MAX_SEQUENCE_ATOMIC_SPLICE_WORK_UNITS,
    MAX_SEQUENCE_AVL_HEIGHT,
};
use crate::source::{SourceCursor, SourceEditError, SourceSnapshotLease, SourceVersion};
use crate::storage::{ArenaBuildOwner, ArenaBuildSession, CandidateBuild, PageArena};
...
pub(crate) const PERSISTENT_SOURCE_FACTS_ROLE_SCHEMA: u32 = 2;
pub(crate) const PERSISTENT_SOURCE_FACTS_ROLE_DESCRIPTOR_BYTES: usize =
    8 + 4 + 4 + 8 + (8 + 8 + 8 + 16 + 1) + (5 * 2 * 8);
const PERSISTENT_SOURCE_FACTS_COVERAGE_CLEAN_EOF: u32 = 1;

/// Canonical sequence content retained into one candidate build.
///
/// The runtime's committed root remains owned by the document. `owner` is one
/// additional journalled retain in the fresh candidate build, never a transfer
/// of the runtime capability.
pub(crate) struct RetainedPersistentSourceFactsRole {
    owner: Option<ArenaBuildOwner>,
    descriptor: [u8; PERSISTENT_SOURCE_FACTS_ROLE_DESCRIPTOR_BYTES],
    record_count: u64,
    canonical_bytes: u64,
    digest: [u8; 32],
...
impl RetainedPersistentSourceFactsRole {
    pub(crate) fn take_owner(&mut self) -> Option<ArenaBuildOwner> {
        self.owner.take()
    }

    pub(crate) const fn descriptor(&self) -> &[u8; PERSISTENT_SOURCE_FACTS_ROLE_DESCRIPTOR_BYTES] {
        &self.descriptor
    }
...
impl PersistentSourceFactsRoot {
    ...
    pub(crate) fn tree_root_id_for_test(&self) -> Option<ArenaId> {
        self.tree.as_ref().and_then(|tree| tree.as_ref().root_id())
    }
    ...
    pub(crate) fn retain_for_publication(
        ...
        expected_parser_profile: ParserProfileId,
        expected_scan_profile: SourceFactsScanProfile,
    ) -> Result<RetainedPersistentSourceFactsRole, SourceFactsAssemblyError> {
        let authority = self.authority_snapshot();
        if self.source != expected_source {
            return Err(SourceFactsAssemblyError::SourceMismatch);
        }
        if self.parser_profile != expected_parser_profile || self.profile != expected_scan_profile {
            return Err(SourceFactsAssemblyError::ProfileMismatch);
        }
        if self.coverage != SourceFactsCoverage::CleanEof {
            return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
        }

        let mut receipt = SequenceMutationReceipt::default();
        let (owner, commitment) = match self.tree.as_ref() {
            Some(tree) => {
                if authority.tree_root != tree.as_ref().root_id() {
                    return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
                }
                ...
                let summary = measure.summary();
                if measure.leaves() != self.page_count
                    || summary.checkpoint_spacing_utf16 != self.profile.checkpoint_spacing_utf16
                    || summary.content_fingerprint_algorithm
                        != self.profile.content_fingerprint_algorithm
                    || summary.checkpoint_count != self.checkpoint_count
                    || summary.segment != self.summary
                    || summary.commitment != self.expected_commitment
                {
                    return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
                }
                (Some(retained.into_owner()), summary.commitment)
            }
            None => {
                if self.page_count != 0
                    || self.checkpoint_count != 0
        ...
        if commitment != authority.ordered_commitment {
            return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
        }
        if self.summary.byte_len
            != u64::try_from(self.source.byte_len())
                .map_err(|_| SourceFactsAssemblyError::CounterExhausted)?
            || self.summary.utf16_len
        ...
        let canonical_bytes =
            persistent_source_facts_canonical_leaf_bytes(self.page_count, self.checkpoint_count)?;
        let descriptor = encode_persistent_source_facts_role_descriptor(
            self.profile,
            self.checkpoint_count,
            self.summary,
            self.expected_commitment,
        ...
        let digest =
            persistent_source_facts_role_digest(self.page_count, canonical_bytes, &descriptor);
        Ok(RetainedPersistentSourceFactsRole {
            owner,
            descriptor,
            record_count: self.page_count,
            canonical_bytes,
            digest,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct PersistentSourceFactsRoleDescriptor {
    profile: SourceFactsScanProfile,
    checkpoint_count: u64,
    summary: SourceFactSegmentSummary,
    commitment: persistent_sequence_codec::SourceFactsSequenceCommitment,
}

/// Authority-bound role claim decoded before a host replay can consume arena
/// ownership.
///
/// The encoded descriptor and role metadata remain the transport truth. This
/// claim is converted into an opaque typed descriptor only after their digest,
/// dimensions, counts, profile, and production admission envelope agree.
pub(crate) struct PersistentSourceFactsHostReplayDescriptorClaim<'descriptor> {
    pub(crate) descriptor_bytes: &'descriptor [u8],
...
/// Typed clean-EOF SourceFacts authority used by the host replay core.
///
/// Its fields stay private so callers cannot construct a target commitment by
/// assembling unchecked scalar claims. A descriptor is Copy only after the
/// schema-owned decoder has authenticated the complete role claim.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct PersistentSourceFactsHostReplayDescriptor {
    role: PersistentSourceFactsRoleDescriptor,
    page_count: u64,
    source_bytes: u64,
    source_utf16: u64,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct ValidatedPersistentSourceFactsRole {
    pub(crate) root: Option<ArenaId>,
    pub(crate) record_count: u64,
}

pub(crate) fn is_persistent_source_facts_sequence_node_payload(payload: &[u8]) -> bool {
    matches!(payload.get(..4), Some(b"SFL2") | Some(b"SFB2"))
}

pub(crate) fn is_persistent_source_facts_sequence_leaf_payload(payload: &[u8]) -> bool {
    payload.get(..4) == Some(b"SFL2")
}

/// Validates one postorder-imported measured node. Calling this exactly once
/// for every sequence node proves the full closure incrementally: leaves bind
/// their exact bytes, and each branch recomputes its associative child
/// commitment and all semantic measures.
pub(crate) fn validate_imported_persistent_source_facts_node(
    arena: &PageArena,
    id: ArenaId,
) -> Result<(), SourceFactsAssemblyError> {
    if !is_persistent_source_facts_sequence_node_payload(arena.payload(id)?) {
        return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
            "imported SourceFacts node has the wrong payload kind",
        ));
    }
    let mut inspection = SequenceInspectionReceipt::default();
    let _ = validate_measured_sequence_node::<persistent_sequence_codec::SourceFactsSequenceSpec>(
        arena,
        id,
        &mut inspection,
    )?;
...
pub(crate) struct PersistentSourceFactsRoleValidation<'descriptor> {
    pub(crate) root: Option<ArenaId>,
    pub(crate) descriptor_bytes: &'descriptor [u8],
    pub(crate) record_count: u64,
    pub(crate) canonical_bytes: u64,
...
fn validate_persistent_source_facts_descriptor_claim(
    ...
    let descriptor = decode_persistent_source_facts_role_descriptor(claim.descriptor_bytes)?;
    if descriptor.summary.byte_len != claim.source_bytes
        || descriptor.summary.utf16_len != claim.source_utf16
        || claim.canonical_bytes
            != persistent_source_facts_canonical_leaf_bytes(
                claim.record_count,
                descriptor.checkpoint_count,
            )?
        || claim.digest
            != persistent_source_facts_role_digest(
                claim.record_count,
                claim.canonical_bytes,
                claim.descriptor_bytes,
            )
    {
        return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
    }
    Ok(PersistentSourceFactsHostReplayDescriptor {
        role: descriptor,
        page_count: claim.record_count,
        source_bytes: claim.source_bytes,
        source_utf16: claim.source_utf16,
    })
}

/// Authenticates a replay descriptor inside the production SourceFacts
/// admission envelope.
//
...
pub(crate) fn validate_persistent_source_facts_host_replay_descriptor(
    ...
    let descriptor = validate_persistent_source_facts_descriptor_claim(claim)?;
    descriptor.validate_host_admission()?;
    Ok(descriptor)
}

pub(crate) fn validate_persistent_source_facts_role(
    arena: &PageArena,
    validation: PersistentSourceFactsRoleValidation<'_>,
) -> Result<ValidatedPersistentSourceFactsRole, SourceFactsAssemblyError> {
    let PersistentSourceFactsRoleValidation {
        root,
        descriptor_bytes,
        record_count,
        canonical_bytes,
    ...
    let descriptor = validate_persistent_source_facts_descriptor_claim(
    ...
    match root {
        Some(root) => {
            ...
            let mut inspection = SequenceInspectionReceipt::default();
            let measure = validate_measured_sequence_node::<
                persistent_sequence_codec::SourceFactsSequenceSpec,
            >(arena, root, &mut inspection)?;
            let summary = measure.summary();
            if measure.leaves() != record_count
                || summary.checkpoint_spacing_utf16
                    != descriptor.role.profile.checkpoint_spacing_utf16
                || summary.content_fingerprint_algorithm
                    != descriptor.role.profile.content_fingerprint_algorithm
                || summary.checkpoint_count != descriptor.role.checkpoint_count
                || summary.segment != descriptor.role.summary
                || summary.commitment != descriptor.role.commitment
            {
                return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
            }
        }
        None => {
            if record_count != 0
                || descriptor.role.checkpoint_count != 0
                || descriptor.role.summary != SourceFactSegmentSummary::default()
                || descriptor.role.commitment
                    != persistent_sequence_codec::SourceFactsSequenceCommitment::empty()
            {
                return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
            }
    ...
    Ok(ValidatedPersistentSourceFactsRole { root, record_count })
}

/// Maximum authenticated AVL height under the production SourceFacts page
/// admission limit.
///
/// An AVL tree of height 21 needs at least 10,946 leaves; height 22 needs
/// 17,711, which exceeds the admitted 15,625 pages.
pub(crate) const PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_HEIGHT: u16 = 21;
/// SourceFacts-specific structural work gate for one atomic host splice.
///
/// This mirrors the measured-sequence proof (`16 * (height + 1)^2`) at the
/// stricter production SourceFacts height rather than merely accepting the
/// outer u64-domain ceiling.
pub(crate) const PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_WORK_UNITS: u64 = {
    let levels = PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_HEIGHT as u64 + 1;
    16 * levels * levels
};
...
impl PersistentSourceFactsHostReplayDescriptor {
    fn is_empty(self) -> bool {
        self.page_count == 0
            && self.role.checkpoint_count == 0
            && self.role.summary == SourceFactSegmentSummary::default()
            && self.role.commitment
                == persistent_sequence_codec::SourceFactsSequenceCommitment::empty()
            && self.source_bytes == 0
            && self.source_utf16 == 0
    }

    fn validate_host_admission(self) -> Result<(), SourceFactsAssemblyError> {
        let page_count = usize::try_from(self.page_count)
            .map_err(|_| SourceFactsAssemblyError::CounterExhausted)?;
        if page_count > SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES {
            return Err(SourceFactsAssemblyError::AdmissionPageLimitExceeded {
                needed: page_count,
                limit: SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES,
            });
        }
        let checkpoint_count = usize::try_from(self.role.checkpoint_count)
            .map_err(|_| SourceFactsAssemblyError::CounterExhausted)?;
        if checkpoint_count > SOURCE_FACT_ROOT_DEFAULT_MAX_CHECKPOINTS {
            return Err(SourceFactsAssemblyError::AdmissionCheckpointLimitExceeded {
                needed: checkpoint_count,
                limit: SOURCE_FACT_ROOT_DEFAULT_MAX_CHECKPOINTS,
            });
        }
        let resident_bytes = source_facts_resident_bytes(checkpoint_count, page_count)?;
        if resident_bytes > SOURCE_FACT_ROOT_DEFAULT_MAX_RESIDENT_BYTES {
            return Err(
                SourceFactsAssemblyError::AdmissionResidentBytesLimitExceeded {
                    needed: resident_bytes,
                    limit: SOURCE_FACT_ROOT_DEFAULT_MAX_RESIDENT_BYTES,
                },
            );
        }
        Ok(())
    ...
    fn validate_measure(
        self,
        measure: SequenceMeasure<persistent_sequence_codec::SourceFactsSequenceSummary>,
    ) -> Result<(), SourceFactsAssemblyError> {
        let summary = measure.summary();
        if measure.leaves() != self.page_count
            || measure.height() > PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_HEIGHT
            || summary.checkpoint_spacing_utf16 != self.role.profile.checkpoint_spacing_utf16
            || summary.content_fingerprint_algorithm
                != self.role.profile.content_fingerprint_algorithm
            || summary.checkpoint_count != self.role.checkpoint_count
            || summary.segment != self.role.summary
            || summary.segment.byte_len != self.source_bytes
            || summary.segment.utf16_len != self.source_utf16
            || summary.commitment != self.role.commitment
        {
            return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
        }
        Ok(())
...
pub(crate) struct PersistentSourceFactsHostReplayWork {
    ...
    atomic_splice: PersistentSourceFactsWork,
    target_validation: PersistentSourceFactsWork,
    total: PersistentSourceFactsWork,
...
/// Validated target root retained by the caller's active host journal.
///
...
impl PersistentSourceFactsHostReplayOutput {
    pub(crate) fn into_parts(
        self,
    ) -> (Option<ArenaBuildOwner>, PersistentSourceFactsHostReplayWork) {
        (self.root, self.work)
    }
}
...
enum PersistentSourceFactsHostReplayPhase {
    ...
    Finishing(SourceFactsMeasuredBuilder),
    Ready {
        replacement: Option<SourceFactsMeasuredBuildRoot>,
    },
    Complete,
    Poisoned,
}

/// Abort-journal-safe replay of one exact SourceFacts v2 delta.
///
/// Every input owner must already belong to the caller's active host build.
/// The base owner is consumed into typed SourceFacts authority, replacement
...
pub(crate) struct PersistentSourceFactsHostReplay {
    ...
    target_descriptor: PersistentSourceFactsHostReplayDescriptor,
    page_range: Range<u64>,
    replacement_page_count: u64,
    phase: PersistentSourceFactsHostReplayPhase,
...
impl PersistentSourceFactsHostReplay {
    pub(crate) fn new(
        ...
        base_descriptor.validate_host_admission()?;
        target_descriptor.validate_host_admission()?;
        if base_descriptor.role.profile != target_descriptor.role.profile {
            return Err(SourceFactsAssemblyError::ProfileMismatch);
        }
        if page_range.start > page_range.end || page_range.end > base_descriptor.page_count {
        ...
        let base = match base_owner {
            Some(owner) => {
                if base_descriptor.page_count == 0 {
                    return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
                }
                ...
                base_descriptor.validate_measure(measure)?;
                Some(validate_measured_sequence_build_owner::<
                    persistent_sequence_codec::SourceFactsSequenceSpec,
                >(
                    session, owner, &mut base_validation_receipt
                )?)
            }
            None => {
                if !base_descriptor.is_empty() {
                    return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
                }
                None
            }
        ...
        Ok(Self {
            ...
            phase: PersistentSourceFactsHostReplayPhase::Accepting {
                builder: None,
                push_active: false,
            },
    ...
    /// Begins adoption of one journal-owned canonical SFL2 leaf.
    pub(crate) fn offer_replacement_leaf(
        ...
        let phase = std::mem::replace(
            &mut self.phase,
            PersistentSourceFactsHostReplayPhase::Poisoned,
        );
        let PersistentSourceFactsHostReplayPhase::Accepting {
            builder,
            push_active: false,
        } = phase
        else {
        ...
        if self.replacement_page_count >= self.target_descriptor.page_count {
            return Err(SourceFactsAssemblyError::PageCountMismatch);
        }
        let mut builder = match builder {
            Some(builder) => builder,
            None => SourceFactsMeasuredBuilder::try_new(session, &mut self.replacement_receipt)?,
        };
        ...
        self.replacement_page_count = self
            ...
            .checked_add(1)
            .ok_or(SourceFactsAssemblyError::CounterExhausted)?;
        self.phase = PersistentSourceFactsHostReplayPhase::Accepting {
    ...
    /// Advances exactly one allocation-granular replacement build quantum.
    pub(crate) fn poll_replacement(
        &mut self,
        session: &mut ArenaBuildSession<'_>,
    ) -> Result<PersistentSourceFactsHostReplayPoll, SourceFactsAssemblyError> {
        let phase = std::mem::replace(
            &mut self.phase,
            PersistentSourceFactsHostReplayPhase::Poisoned,
        );
        match phase {
            PersistentSourceFactsHostReplayPhase::Accepting {
                builder: Some(mut builder),
                push_active: true,
            } => {
                let progress = builder.poll_push(session, &mut self.replacement_receipt)?;
                let complete = progress == ResumableSequenceProgress::Complete;
                self.phase = PersistentSourceFactsHostReplayPhase::Accepting {
                    builder: Some(builder),
            ...
            PersistentSourceFactsHostReplayPhase::Finishing(mut builder) => {
                ...
                if progress == ResumableSequenceProgress::Complete {
                    ...
                    self.phase = PersistentSourceFactsHostReplayPhase::Ready {
                        replacement: Some(replacement),
                    };
                    Ok(PersistentSourceFactsHostReplayPoll::Complete)
                } else {
                    self.phase = PersistentSourceFactsHostReplayPhase::Finishing(builder);
                    Ok(PersistentSourceFactsHostReplayPoll::Pending)
                }
            }
            _ => Err(SourceFactsAssemblyError::BuilderPoisoned),
        }
    }

    /// Closes the replacement stream. Zero leaves become `None` directly;
    /// no sentinel or empty arena node is allocated.
    pub(crate) fn finish_replacement(
        ...
        let retained_pages = self
            .base_descriptor
            .page_count
            .checked_sub(self.page_range.end - self.page_range.start)
            .ok_or(SourceFactsAssemblyError::PageCountMismatch)?;
        let expected_target_pages = retained_pages
            .checked_add(self.replacement_page_count)
            .ok_or(SourceFactsAssemblyError::CounterExhausted)?;
        if expected_target_pages != self.target_descriptor.page_count {
            return Err(SourceFactsAssemblyError::PageCountMismatch);
        }
        ...
        match builder {
            ...
            None => {
                self.phase = PersistentSourceFactsHostReplayPhase::Ready { replacement: None };
                Ok(PersistentSourceFactsHostReplayPoll::Complete)
            }
        }
    }

    /// Performs the single bounded owned-root splice and validates the exact
    /// target descriptor before releasing an owner to the host program.
    ///
    /// This operation is deliberately not a fuelled state machine. Both the
    /// generic u64-domain ceiling and the stricter SourceFacts admission
    /// ceiling are checked from the complete atomic receipt.
    pub(crate) fn complete(
        &mut self,
        session: &mut ArenaBuildSession<'_>,
    ) -> Result<PersistentSourceFactsHostReplayOutput, SourceFactsAssemblyError> {
        let phase = std::mem::replace(
            &mut self.phase,
            PersistentSourceFactsHostReplayPhase::Poisoned,
        );
        let PersistentSourceFactsHostReplayPhase::Ready { replacement } = phase else {
            return Err(SourceFactsAssemblyError::BuilderPoisoned);
        };

        let target_root = match self.base.take() {
            Some(base) => splice_measured_sequence_build_root_atomic::<
                persistent_sequence_codec::SourceFactsSequenceSpec,
            >(
                session,
                base,
            ...
            None => {
                if self.page_range != (0..0) {
                    return Err(SourceFactsAssemblyError::PageCountMismatch);
                }
                replacement
            }
        };
        let atomic_splice_work_units =
            persistent_source_facts_atomic_splice_work_units(self.splice_receipt)?;
        if self.splice_receipt.maximum_atomic_height
            > PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_HEIGHT
            || atomic_splice_work_units > PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_WORK_UNITS
            || self.splice_receipt.maximum_atomic_height > MAX_SEQUENCE_AVL_HEIGHT
            || atomic_splice_work_units > MAX_SEQUENCE_ATOMIC_SPLICE_WORK_UNITS
        {
            return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
                "atomic SourceFacts replay exceeded its authenticated work envelope",
            ));
        }

        let target_owner = target_root.map(MeasuredSequenceBuildRoot::into_owner);
        match target_owner.as_ref() {
            Some(owner) => {
                session.validate_owner(owner)?;
                let measure = validate_measured_sequence_node::<
                    persistent_sequence_codec::SourceFactsSequenceSpec,
                ...
                self.target_descriptor.validate_measure(measure)?;
            }
            None => {
                if !self.target_descriptor.is_empty() {
                    return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
                }
            }
        }
        ...
        let base_validation =
            PersistentSourceFactsWork::from_receipt(self.base_validation_receipt, 0);
        let replacement_build =
            PersistentSourceFactsWork::from_receipt(self.replacement_receipt, 0);
        let atomic_splice = PersistentSourceFactsWork::from_receipt(self.splice_receipt, 0);
        let target_validation =
            PersistentSourceFactsWork::from_receipt(self.target_validation_receipt, 0);
        let total = base_validation
            .checked_add(replacement_build)
            .and_then(|work| work.checked_add(atomic_splice))
            .and_then(|work| work.checked_add(target_validation))
            .ok_or(SourceFactsAssemblyError::CounterExhausted)?;
        let work = PersistentSourceFactsHostReplayWork {
            base_validation,
            replacement_build,
            atomic_splice,
            target_validation,
            total,
            atomic_splice_work_units,
        };
        self.phase = PersistentSourceFactsHostReplayPhase::Complete;
        Ok(PersistentSourceFactsHostReplayOutput {
            root: target_owner,
            work,
        })
    }
...
fn persistent_source_facts_atomic_splice_work_units(
    ...
    receipt
        .inspection
        .node_headers_decoded
        .checked_add(receipt.inspection.summary_combinations)
        .and_then(|work| {
            u64::try_from(receipt.nodes_visited)
                .ok()
                .and_then(|visited| work.checked_add(visited))
        })
        .and_then(|work| {
            u64::try_from(receipt.branches_allocated)
                .ok()
                .and_then(|branches| work.checked_add(branches))
        })
        .ok_or(SourceFactsAssemblyError::CounterExhausted)
}

pub(crate) fn persistent_source_facts_leaf_record_at(
    arena: &PageArena,
    root: Option<ArenaId>,
    ordinal: u64,
) -> Result<Option<&[u8]>, SourceFactsAssemblyError> {
    let Some(root) = root else {
        return Ok(None);
    };
    let mut inspection = SequenceInspectionReceipt::default();
    let Some(located) = locate_measured_sequence_leaf_with_prefix::<
        persistent_sequence_codec::SourceFactsSequenceSpec,
    >(arena, root, ordinal, &mut inspection)?
    else {
        return Ok(None);
    };
    let payload = arena.payload(located.id)?;
    let decoded =
        persistent_sequence_codec::decode_source_fact_sequence_leaf(payload, &mut inspection.spec)?
            .ok_or(SourceFactsAssemblyError::CorruptPersistentSequence(
                "located SourceFacts record is not a sequence leaf",
            ))?;
    if decoded.summary != located.summary {
        return Err(SourceFactsAssemblyError::CanonicalSummaryMismatch);
    }
    Ok(Some(payload))
}
...
const _: () = {
    assert!(SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES >= 10_946);
    assert!(SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES < 17_711);
    assert!(PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_HEIGHT < MAX_SEQUENCE_AVL_HEIGHT);
    assert!(
        PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_WORK_UNITS
            < MAX_SEQUENCE_ATOMIC_SPLICE_WORK_UNITS
    );
};

fn persistent_source_facts_canonical_leaf_bytes(
    page_count: u64,
    checkpoint_count: u64,
) -> Result<u64, SourceFactsAssemblyError> {
    let fixed = u64::try_from(persistent_sequence_codec::SOURCE_FACT_SEQUENCE_LEAF_FIXED_BYTES)
        .map_err(|_| SourceFactsAssemblyError::CounterExhausted)?;
    let checkpoint = u64::try_from(persistent_sequence_codec::SOURCE_FACT_SEGMENT_ENCODED_BYTES)
        .map_err(|_| SourceFactsAssemblyError::CounterExhausted)?;
    page_count
        .checked_mul(fixed)
        .and_then(|bytes| {
            checkpoint_count
                .checked_mul(checkpoint)
                .and_then(|checkpoints| bytes.checked_add(checkpoints))
        })
        .ok_or(SourceFactsAssemblyError::CounterExhausted)
}

fn persistent_source_facts_role_digest(
    record_count: u64,
    canonical_bytes: u64,
    descriptor: &[u8],
) -> [u8; 32] {
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"flark.candidate.source-facts-sequence-role.v2\0");
    hasher.update(&PERSISTENT_SOURCE_FACTS_ROLE_SCHEMA.to_le_bytes());
...
fn encode_persistent_source_facts_role_descriptor(
    profile: SourceFactsScanProfile,
    checkpoint_count: u64,
    summary: SourceFactSegmentSummary,
    commitment: persistent_sequence_codec::SourceFactsSequenceCommitment,
) -> [u8; PERSISTENT_SOURCE_FACTS_ROLE_DESCRIPTOR_BYTES] {
    let mut output = [0_u8; PERSISTENT_SOURCE_FACTS_ROLE_DESCRIPTOR_BYTES];
    let mut cursor = 0;
    ...
    write(&profile.checkpoint_spacing_utf16.to_le_bytes());
    write(&profile.content_fingerprint_algorithm.to_le_bytes());
    write(&PERSISTENT_SOURCE_FACTS_COVERAGE_CLEAN_EOF.to_le_bytes());
    write(&checkpoint_count.to_le_bytes());
    write(&summary.byte_len.to_le_bytes());
    write(&summary.utf16_len.to_le_bytes());
    write(&summary.logical_line_breaks.to_le_bytes());
    for word in summary.rolling_hash.words {
        write(&word.to_le_bytes());
    }
    write(&[u8::from(summary.starts_with_line_feed)
        | (u8::from(summary.ends_with_carriage_return) << 1)]);
    for value in commitment.values {
        write(&value.to_le_bytes());
    }
    for power in commitment.powers {
        write(&power.to_le_bytes());
    }
    debug_assert_eq!(cursor, output.len());
    output
}

fn decode_persistent_source_facts_role_descriptor(
    bytes: &[u8],
) -> Result<PersistentSourceFactsRoleDescriptor, SourceFactsAssemblyError> {
    if bytes.len() != PERSISTENT_SOURCE_FACTS_ROLE_DESCRIPTOR_BYTES {
        return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
            "SourceFacts role descriptor has the wrong length",
        ));
    }
    let mut cursor = 0;
    let read_u32 = |cursor: &mut usize| -> Result<u32, SourceFactsAssemblyError> {
        let end = cursor
            .checked_add(4)
            .ok_or(SourceFactsAssemblyError::CounterExhausted)?;
    ...
    let read_u64 = |cursor: &mut usize| -> Result<u64, SourceFactsAssemblyError> {
        let end = cursor
            .checked_add(8)
            .ok_or(SourceFactsAssemblyError::CounterExhausted)?;
    ...
    let checkpoint_spacing_utf16 = read_u64(&mut cursor)?;
    let content_fingerprint_algorithm = read_u32(&mut cursor)?;
    let coverage = read_u32(&mut cursor)?;
    let checkpoint_count = read_u64(&mut cursor)?;
    if coverage != PERSISTENT_SOURCE_FACTS_COVERAGE_CLEAN_EOF
        || content_fingerprint_algorithm != SOURCE_CONTENT_FINGERPRINT_ALGORITHM
    {
        return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
            "SourceFacts role descriptor profile or coverage changed",
        ));
    }
    let spacing = usize::try_from(checkpoint_spacing_utf16)
        .map_err(|_| SourceFactsAssemblyError::CounterExhausted)?;
    let profile = SourceFactsScanProfile::new(spacing)
        .map_err(|_| SourceFactsAssemblyError::InvalidProfile)?;
    let byte_len = read_u64(&mut cursor)?;
    let utf16_len = read_u64(&mut cursor)?;
    let logical_line_breaks = read_u64(&mut cursor)?;
    let mut words = [0_u32; 4];
    for word in &mut words {
        *word = read_u32(&mut cursor)?;
    }
    let flags = *bytes
        .get(cursor)
        .ok_or(SourceFactsAssemblyError::CorruptPersistentSequence(
            "SourceFacts role descriptor flags are missing",
        ))?;
    cursor += 1;
    if flags & !0b11 != 0 {
        return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
            "SourceFacts role descriptor flags changed",
        ));
    }
    let summary = SourceFactSegmentSummary {
        byte_len,
    ...
    if !summary.is_structurally_valid() {
        return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
            "SourceFacts role descriptor summary is invalid",
        ));
    }
    let mut commitment = persistent_sequence_codec::SourceFactsSequenceCommitment::empty();
    for value in &mut commitment.values {
        *value = read_u64(&mut cursor)?;
        if *value >= persistent_sequence_codec::COMMITMENT_MODULUS {
            return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
                "SourceFacts role commitment value is noncanonical",
            ));
        }
    }
    for power in &mut commitment.powers {
        *power = read_u64(&mut cursor)?;
        if *power == 0 || *power >= persistent_sequence_codec::COMMITMENT_MODULUS {
            return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
                "SourceFacts role commitment power is noncanonical",
            ));
        }
    }
    if cursor != bytes.len() {
        return Err(SourceFactsAssemblyError::CorruptPersistentSequence(
            "SourceFacts role descriptor has trailing bytes",
        ));
    }
    Ok(PersistentSourceFactsRoleDescriptor {
        profile,
        checkpoint_count,
        summary,
        commitment,
...
mod tests {
    ...
    fn host_replay_descriptor(
        root: &PersistentSourceFactsRoot,
    ) -> PersistentSourceFactsHostReplayDescriptor {
        let descriptor_bytes = encode_persistent_source_facts_role_descriptor(
            root.profile,
            root.checkpoint_count,
            root.summary,
            root.expected_commitment,
        );
        let canonical_bytes =
            persistent_source_facts_canonical_leaf_bytes(root.page_count, root.checkpoint_count)
                .expect("canonical bytes");
        let digest = persistent_source_facts_role_digest(
            root.page_count,
            canonical_bytes,
            &descriptor_bytes,
        );
        validate_persistent_source_facts_host_replay_descriptor(
            PersistentSourceFactsHostReplayDescriptorClaim {
                descriptor_bytes: &descriptor_bytes,
                record_count: root.page_count,
                canonical_bytes,
                digest,
                source_bytes: root.summary.byte_len,
                source_utf16: root.summary.utf16_len,
            },
        )
        .expect("host replay descriptor")
    }

    fn persistent_leaf_payloads(
        arena: &PageArena,
        root: &PersistentSourceFactsRoot,
        range: Range<u64>,
    ) -> Vec<Vec<u8>> {
        range
            .map(|ordinal| {
                persistent_source_facts_leaf_record_at(arena, root.tree_root_id_for_test(), ordinal)
                    .expect("persistent leaf lookup")
                    .expect("persistent leaf")
                    .to_vec()
            })
            .collect()
    }

    fn offer_host_replay_leaf(
        replay: &mut PersistentSourceFactsHostReplay,
        session: &mut ArenaBuildSession<'_>,
        payload: &[u8],
    ) {
        let leaf = session
            .allocate(payload, &[])
            .expect("allocate imported replay leaf");
        replay
            .offer_replacement_leaf(session, leaf)
            .expect("offer replay leaf");
        while replay.poll_replacement(session).expect("poll replay leaf")
            == PersistentSourceFactsHostReplayPoll::Pending
        {}
    }

    fn finish_host_replay(
        replay: &mut PersistentSourceFactsHostReplay,
        session: &mut ArenaBuildSession<'_>,
    ) {
        let mut progress = replay
            .finish_replacement(session)
            .expect("finish replay replacement");
        while progress == PersistentSourceFactsHostReplayPoll::Pending {
            progress = replay
                .poll_replacement(session)
                .expect("poll replay finish");
        }
    }

    fn one_page_host_replay_work(
        source_bytes: usize,
    ) -> (u64, PersistentSourceFactsHostReplayWork) {
        let spacing = 2;
        ...
        let replacement_ordinal =
            u64::try_from((boundaries.len() - 1) / 2).expect("replacement ordinal");
        let replacement_index = usize::try_from(replacement_ordinal).expect("replacement index");
        let mut target_text = source;
        target_text.replace_range(
            boundaries[replacement_index]..boundaries[replacement_index] + 1,
            "b",
        );
        ...
        let target = promote_persistent_source_facts(&mut arena, certified(&target_text, spacing));
        let base_descriptor = host_replay_descriptor(&base);
        let target_descriptor = host_replay_descriptor(&target);
        let base_root = base.tree_root_id_for_test().expect("base root");
        let payload = persistent_leaf_payloads(
            &arena,
            &target,
            replacement_ordinal..replacement_ordinal + 1,
        )
        .pop()
        ...
        let work = {
            let mut session = arena.begin_build().expect("bounded replay build");
            let base_owner = session.retain(base_root).expect("retain base");
            let mut replay = PersistentSourceFactsHostReplay::new(
                &session,
                Some(base_owner),
                base_descriptor,
                replacement_ordinal..replacement_ordinal + 1,
            ...
            .expect("begin bounded replay");
            offer_host_replay_leaf(&mut replay, &mut session, &payload);
            finish_host_replay(&mut replay, &mut session);
            let (_root, work) = replay
                .complete(&mut session)
                .expect("complete bounded replay")
                .into_parts();
            drop(session);
            work
        };
        ...
        assert!(target.release(&mut arena).is_ok());
        drain_arena(&mut arena);
    ...
    fn clean_promotion_anchors_tree_commitment_to_certified_page_order() {
        ...
        let retained_inspection = {
            let mut session = arena.begin_build().expect("publication retain");
            let inspection = {
                let retained = root
                    .retain_for_publication(
                        &mut session,
                        root.source,
                        root.parser_profile,
                        root.profile,
                    )
                    .expect("independently anchored retain");
                assert_eq!(retained.record_count(), root.page_count);
                retained.inspection()
            };
            let build = session.suspend().expect("suspend retained root");
            arena.abort_build(build).expect("abort test retain");
            inspection
        };
        assert_eq!(retained_inspection.node_headers_decoded, 3);
        assert_eq!(retained_inspection.summary_combinations, 1);
        assert!(
            retained_inspection.spec.payload_bytes_inspected
                <= 3 * u64::try_from(ARENA_PAGE_BYTES).expect("page size")
        );
        assert!(
            retained_inspection.spec.spec_items_hashed
    ...
    #[test]
    fn host_replay_descriptor_authenticates_claim_and_applies_every_admission_dimension() {
        let profile = SourceFactsScanProfile::new(2).expect("profile");
        let summary = SourceFactSegmentSummary::default();
        let commitment = SourceFactsSequenceCommitment::empty();
        let descriptor_bytes =
            encode_persistent_source_facts_role_descriptor(profile, 0, summary, commitment);
        let canonical_bytes =
            persistent_source_facts_canonical_leaf_bytes(0, 0).expect("canonical empty bytes");
        let digest = persistent_source_facts_role_digest(0, canonical_bytes, &descriptor_bytes);

        assert!(validate_persistent_source_facts_host_replay_descriptor(
            PersistentSourceFactsHostReplayDescriptorClaim {
                descriptor_bytes: &descriptor_bytes,
                record_count: 0,
                canonical_bytes,
        ...
        assert!(matches!(
            ...
            Err(SourceFactsAssemblyError::CanonicalSummaryMismatch)
        ));
        ...
        let excessive_pages =
            u64::try_from(SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES + 1).expect("page count");
        let excessive_page_bytes = persistent_source_facts_canonical_leaf_bytes(excessive_pages, 0)
            .expect("excessive-page canonical bytes");
        let excessive_page_digest = persistent_source_facts_role_digest(
            excessive_pages,
            excessive_page_bytes,
            &descriptor_bytes,
        );
        assert!(matches!(
            validate_persistent_source_facts_host_replay_descriptor(
                PersistentSourceFactsHostReplayDescriptorClaim {
                    descriptor_bytes: &descriptor_bytes,
                    record_count: excessive_pages,
                    canonical_bytes: excessive_page_bytes,
                    digest: excessive_page_digest,
                    source_bytes: 0,
                    source_utf16: 0,
                },
            ),
            Err(SourceFactsAssemblyError::AdmissionPageLimitExceeded {
                needed,
                limit: SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES,
            }) if needed == SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES + 1
        ));

        let excessive_checkpoints =
            u64::try_from(SOURCE_FACT_ROOT_DEFAULT_MAX_CHECKPOINTS + 1).expect("checkpoint count");
        let checkpoint_descriptor = encode_persistent_source_facts_role_descriptor(
            profile,
            excessive_checkpoints,
            summary,
            commitment,
        );
        let excessive_checkpoint_bytes =
            persistent_source_facts_canonical_leaf_bytes(0, excessive_checkpoints)
                .expect("excessive-checkpoint canonical bytes");
        let excessive_checkpoint_digest = persistent_source_facts_role_digest(
            0,
            excessive_checkpoint_bytes,
            &checkpoint_descriptor,
        );
        assert!(matches!(
            validate_persistent_source_facts_host_replay_descriptor(
                PersistentSourceFactsHostReplayDescriptorClaim {
                    descriptor_bytes: &checkpoint_descriptor,
                    record_count: 0,
                    canonical_bytes: excessive_checkpoint_bytes,
                    digest: excessive_checkpoint_digest,
                    source_bytes: 0,
                    source_utf16: 0,
                },
            ),
            Err(SourceFactsAssemblyError::AdmissionCheckpointLimitExceeded {
                needed,
                limit: SOURCE_FACT_ROOT_DEFAULT_MAX_CHECKPOINTS,
            }) if needed == SOURCE_FACT_ROOT_DEFAULT_MAX_CHECKPOINTS + 1
        ));

        let maximum_resident = source_facts_resident_bytes(
            SOURCE_FACT_ROOT_DEFAULT_MAX_CHECKPOINTS,
            SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES,
        )
        .expect("maximum admitted resident bytes");
        let maximum_counts = PersistentSourceFactsHostReplayDescriptor {
            role: PersistentSourceFactsRoleDescriptor {
                profile,
                checkpoint_count: u64::try_from(SOURCE_FACT_ROOT_DEFAULT_MAX_CHECKPOINTS)
                    .expect("checkpoint count"),
                summary,
                commitment,
            },
            page_count: u64::try_from(SOURCE_FACT_ROOT_DEFAULT_MAX_PAGES).expect("page count"),
            source_bytes: 0,
            source_utf16: 0,
        };
        let admission = maximum_counts.validate_host_admission();
        if maximum_resident > SOURCE_FACT_ROOT_DEFAULT_MAX_RESIDENT_BYTES {
            assert_eq!(
                admission,
                Err(
                    SourceFactsAssemblyError::AdmissionResidentBytesLimitExceeded {
                        needed: maximum_resident,
                        limit: SOURCE_FACT_ROOT_DEFAULT_MAX_RESIDENT_BYTES,
                    },
                )
            );
    ...
    #[test]
    fn host_replay_matches_clean_target_and_stays_inside_source_facts_atomic_gate() {
        let source = "abcd".repeat(600);
        let spacing = 2;
        let base_certified = certified(&source, spacing);
        let mut boundaries = Vec::with_capacity(base_certified.facts().pages().len() + 1);
        ...
        let replacement_payloads =
            persistent_leaf_payloads(&arena, &clean_target, replacement_range.clone());
        assert_eq!(replacement_payloads.len(), 4);
        let base_descriptor = host_replay_descriptor(&base);
        let target_descriptor = host_replay_descriptor(&clean_target);
        let base_root = base.tree_root_id_for_test().expect("base root");

        let (build, replay_root, work) = {
            let mut session = arena.begin_build().expect("host replay build");
            let base_owner = session
                .retain(base_root)
                .expect("retain exact installed base");
            let mut replay = PersistentSourceFactsHostReplay::new(
                &session,
                Some(base_owner),
                base_descriptor,
            ...
            .expect("begin host replay");
            for payload in &replacement_payloads {
                offer_host_replay_leaf(&mut replay, &mut session, payload);
            }
            finish_host_replay(&mut replay, &mut session);
            let (root, work) = replay
                .complete(&mut session)
                .expect("complete host replay")
                .into_parts();
            let root = root.expect("nonempty replay target");
            let mut typed_receipt = SequenceMutationReceipt::default();
            let typed = validate_measured_sequence_build_owner::<SourceFactsSequenceSpec>(
                &session,
                root,
                &mut typed_receipt,
            )
            .expect("typed replay target");
            (session.suspend().expect("suspend host replay"), typed, work)
        };
        let replay_root = seal_source_fact_sequence(&mut arena, build, replay_root);

        let mut replay_inspection = SequenceInspectionReceipt::default();
        let replay_measure = replay_root
            .as_ref()
            .summary(&arena, &mut replay_inspection)
            .expect("replay summary")
            .expect("replay root");
        let mut clean_inspection = SequenceInspectionReceipt::default();
        let clean_measure = clean_target
            .tree
            .as_ref()
            .expect("clean target root")
            .as_ref()
            .summary(&arena, &mut clean_inspection)
            .expect("clean summary")
            .expect("clean root");
        assert_eq!(replay_measure, clean_measure);
        for ordinal in 0..clean_target.page_count {
            let replay_leaf = replay_root
                .as_ref()
                .locate_leaf_with_prefix(&arena, ordinal, &mut SequenceInspectionReceipt::default())
                .expect("replay leaf lookup")
                .expect("replay leaf")
                .id;
            let clean_payload = persistent_source_facts_leaf_record_at(
                &arena,
                clean_target.tree_root_id_for_test(),
                ordinal,
            )
            .expect("clean leaf lookup")
            .expect("clean leaf");
            assert_eq!(
                arena.payload(replay_leaf).expect("replay payload"),
                clean_payload
            );
        }

        assert_eq!(work.replacement_build().leaves_adopted(), 4);
        assert_eq!(work.atomic_splice().leaves_deleted(), 4);
        assert_eq!(
            work.atomic_splice().leaves_reused(),
            usize::try_from(base.page_count - 4).expect("reused pages")
        );
        assert_eq!(work.base_validation().committed_leaves_retained(), 0);
        assert_eq!(work.atomic_splice().committed_leaves_retained(), 0);
        assert!(work.target_validation().node_headers_decoded() > 0);
        assert!(
            work.atomic_splice().maximum_atomic_height()
                <= PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_HEIGHT
        );
        assert!(
            work.atomic_splice_work_units()
                <= PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_WORK_UNITS
        );
        assert_eq!(
            work.total(),
            work.base_validation()
                .checked_add(work.replacement_build())
                .and_then(|total| total.checked_add(work.atomic_splice()))
                .and_then(|total| total.checked_add(work.target_validation()))
                .expect("complete replay work")
        );

        assert!(replay_root.release(&mut arena).is_ok());
        assert!(base.release(&mut arena).is_ok());
        assert!(clean_target.release(&mut arena).is_ok());
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, 0);
    ...
    #[test]
    fn host_replay_work_tracks_changed_pages_and_authenticated_height_not_document_pages() {
        let (small_pages, small) = one_page_host_replay_work(4_096);
        let (large_pages, large) = one_page_host_replay_work(65_536);
        assert!(large_pages > small_pages * 8);
        for (pages, work) in [(small_pages, small), (large_pages, large)] {
            assert_eq!(work.replacement_build().leaves_adopted(), 1);
            assert_eq!(work.atomic_splice().leaves_deleted(), 1);
            assert_eq!(
                work.atomic_splice().leaves_reused(),
                usize::try_from(pages - 1).expect("reused pages")
            );
            let levels = u64::from(work.atomic_splice().maximum_atomic_height()) + 1;
            let height_bound = 16 * levels * levels;
            assert!(work.atomic_splice_work_units() <= height_bound);
            assert!(
                work.atomic_splice_work_units()
                    <= PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_WORK_UNITS
            );
    ...
    #[test]
    fn host_replay_rejects_an_atomic_receipt_above_the_source_facts_gate() {
        let spacing = 2;
        let mut arena = PageArena::new(ArenaLimits {
            max_slots: 4_096,
        ...
        .expect("arena");
        let base =
            promote_persistent_source_facts(&mut arena, certified(&"a".repeat(2_000), spacing));
        let descriptor = host_replay_descriptor(&base);
        let base_root = base.tree_root_id_for_test().expect("base root");
        drain_arena(&mut arena);
        let baseline = arena.metrics();

        {
            let mut session = arena.begin_build().expect("gate build");
            let owner = session.retain(base_root).expect("retain base");
            let mut replay = PersistentSourceFactsHostReplay::new(
                &session,
                Some(owner),
                descriptor,
            ...
            .expect("begin gate replay");
            finish_host_replay(&mut replay, &mut session);
            replay.splice_receipt.nodes_visited =
                usize::try_from(PERSISTENT_SOURCE_FACTS_HOST_REPLAY_MAX_ATOMIC_WORK_UNITS + 1)
                    .expect("test work units");
            assert!(matches!(
                replay.complete(&mut session),
        ...
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
            arena.metrics().live_payload_bytes,
            baseline.live_payload_bytes
        );
        assert!(arena.payload(base_root).is_ok());
    ...
    #[test]
    fn host_replay_uses_none_for_empty_roots_and_supports_one_leaf_insertion() {
        let spacing = 2;
        let mut arena = PageArena::new(ArenaLimits {
            max_slots: 4_096,
            max_live_payload_bytes: 16 * 1024 * 1024,
        ...
        .expect("arena");
        let empty = promote_persistent_source_facts(&mut arena, certified("", spacing));
        let one = promote_persistent_source_facts(&mut arena, certified("ab", spacing));
        assert_eq!(empty.page_count, 0);
        assert!(empty.tree_root_id_for_test().is_none());
        assert_eq!(one.page_count, 1);
        let one_root = one.tree_root_id_for_test().expect("one-page root");
        let one_payload = persistent_leaf_payloads(&arena, &one, 0..1)
            .pop()
            .expect("one leaf");
        let empty_descriptor = host_replay_descriptor(&empty);
        let one_descriptor = host_replay_descriptor(&one);
        drain_arena(&mut arena);
        let baseline = arena.metrics();

        let insertion_work = {
            let mut session = arena.begin_build().expect("insertion build");
            let mut replay = PersistentSourceFactsHostReplay::new(
                &session,
                None,
                empty_descriptor,
            ...
            .expect("empty-base replay");
            offer_host_replay_leaf(&mut replay, &mut session, &one_payload);
            finish_host_replay(&mut replay, &mut session);
            let (root, work) = replay
                .complete(&mut session)
                .expect("one-leaf replay")
                .into_parts();
            let root = root.expect("one-leaf target owner");
            let _wrapper = session
                .allocate(b"virtual-source-facts-root", &[root.id()])
                .expect("virtual ordinal consumer");
            session.release(root).expect("transfer replay owner");
            let build = session.suspend().expect("suspend insertion");
            arena.abort_build(build).expect("abort insertion");
            work
        };
        drain_arena(&mut arena);
        assert_eq!(insertion_work.replacement_build().leaves_adopted(), 1);
        assert_eq!(insertion_work.atomic_splice_work_units(), 0);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
            arena.metrics().live_payload_bytes,
            baseline.live_payload_bytes
        ...
        let deletion_work = {
            let mut session = arena.begin_build().expect("deletion build");
            let base_owner = session.retain(one_root).expect("retain one-page base");
            let mut replay = PersistentSourceFactsHostReplay::new(
                &session,
                Some(base_owner),
                one_descriptor,
            ...
            .expect("one-page base replay");
            finish_host_replay(&mut replay, &mut session);
            let (root, work) = replay
                .complete(&mut session)
                .expect("empty-target replay")
                .into_parts();
            assert!(root.is_none(), "empty target must not allocate a sentinel");
            drop(session);
            work
        };
        drain_arena(&mut arena);
        assert_eq!(deletion_work.replacement_build().leaves_adopted(), 0);
        assert_eq!(deletion_work.atomic_splice().leaves_deleted(), 1);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
            arena.metrics().live_payload_bytes,
            baseline.live_payload_bytes
        );
        assert!(arena.payload(one_root).is_ok());

        assert!(empty.release(&mut arena).is_ok());
        assert!(one.release(&mut arena).is_ok());
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, 0);
    }

    #[test]
    fn host_replay_rejects_wrong_base_range_and_profile_without_mutation() {
        let spacing = 2;
        let mut arena = PageArena::new(ArenaLimits {
            max_slots: 8_192,
            max_live_payload_bytes: 32 * 1024 * 1024,
        ...
        let base =
            promote_persistent_source_facts(&mut arena, certified(&"a".repeat(2_000), spacing));
        let other =
            promote_persistent_source_facts(&mut arena, certified(&"b".repeat(2_000), spacing));
        let base_descriptor = host_replay_descriptor(&base);
        let target_descriptor = host_replay_descriptor(&base);
        let base_root = base.tree_root_id_for_test().expect("base root");
        let other_root = other.tree_root_id_for_test().expect("other root");
        drain_arena(&mut arena);
        let baseline = arena.metrics();

        {
            let mut session = arena.begin_build().expect("wrong-base build");
            let wrong_owner = session.retain(other_root).expect("retain wrong base");
            assert!(matches!(
                PersistentSourceFactsHostReplay::new(
                    &session,
                    Some(wrong_owner),
                    base_descriptor,
                    0..0,
            ...
            let build = session.suspend().expect("suspend wrong-base build");
            arena.abort_build(build).expect("abort wrong-base build");
        }
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
        ...
        {
            let mut session = arena.begin_build().expect("wrong-range build");
            let base_owner = session.retain(base_root).expect("retain base");
            assert!(matches!(
                PersistentSourceFactsHostReplay::new(
                    &session,
                ...
                Err(SourceFactsAssemblyError::PageCountMismatch)
            ));
            let build = session.suspend().expect("suspend wrong-range build");
            arena.abort_build(build).expect("abort wrong-range build");
        }
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
        ...
        {
            let mut wrong_profile = target_descriptor;
            wrong_profile.role.profile =
                SourceFactsScanProfile::new(spacing + 2).expect("wrong profile");
            let mut session = arena.begin_build().expect("wrong-profile build");
            let base_owner = session.retain(base_root).expect("retain base");
            assert!(matches!(
                PersistentSourceFactsHostReplay::new(
                    &session,
                ...
                Err(SourceFactsAssemblyError::ProfileMismatch)
            ));
            let build = session.suspend().expect("suspend wrong-profile build");
            arena.abort_build(build).expect("abort wrong-profile build");
        }
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
        ...
        assert!(arena.payload(base_root).is_ok());
        assert!(arena.payload(other_root).is_ok());

        assert!(base.release(&mut arena).is_ok());
        assert!(other.release(&mut arena).is_ok());
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, 0);
    }

    #[test]
    fn host_replay_rejects_commitment_and_leaf_corruption_and_aborts_partial_work() {
        let spacing = 2;
        let mut arena = PageArena::new(ArenaLimits {
            max_slots: 16_384,
            max_live_payload_bytes: 64 * 1024 * 1024,
        ...
        let target =
            promote_persistent_source_facts(&mut arena, certified(&"b".repeat(2_000), spacing));
        assert_eq!(base.page_count, target.page_count);
        assert!(target.page_count > 2);
        let base_descriptor = host_replay_descriptor(&base);
        let target_descriptor = host_replay_descriptor(&target);
        let base_root = base.tree_root_id_for_test().expect("base root");
        let target_payloads = persistent_leaf_payloads(&arena, &target, 0..target.page_count);
        drain_arena(&mut arena);
        let baseline = arena.metrics();

        {
            let mut bad_commitment = target_descriptor;
            bad_commitment.role.commitment.values[0] =
                (bad_commitment.role.commitment.values[0] + 1) % COMMITMENT_MODULUS;
            let mut session = arena.begin_build().expect("bad-commitment build");
            let base_owner = session.retain(base_root).expect("retain base");
            let mut replay = PersistentSourceFactsHostReplay::new(
                &session,
                Some(base_owner),
            ...
            .expect("begin bad-commitment replay");
            ...
            drop(replay);
            let build = session.suspend().expect("suspend bad commitment");
            arena.abort_build(build).expect("abort bad commitment");
        }
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
        ...
        {
            let mut corrupt_payload = target_payloads[0].clone();
            corrupt_payload[22] ^= 0x80;
            let mut session = arena.begin_build().expect("corrupt-leaf build");
            let base_owner = session.retain(base_root).expect("retain base");
            let mut replay = PersistentSourceFactsHostReplay::new(
                &session,
                Some(base_owner),
            ...
            .expect("begin corrupt-leaf replay");
            let corrupt_leaf = session
                .allocate(&corrupt_payload, &[])
                .expect("allocate corrupt leaf");
            assert!(matches!(
                replay.offer_replacement_leaf(&mut session, corrupt_leaf),
                Err(SourceFactsAssemblyError::PageDigestMismatch)
                    | Err(SourceFactsAssemblyError::CorruptPersistentSequence(_))
            ));
            drop(replay);
            let build = session.suspend().expect("suspend corrupt leaf");
            arena.abort_build(build).expect("abort corrupt leaf");
        }
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
        ...
        {
            let mut session = arena.begin_build().expect("partial replay build");
            let base_owner = session.retain(base_root).expect("retain base");
            let mut replay = PersistentSourceFactsHostReplay::new(
                &session,
                Some(base_owner),
            ...
            offer_host_replay_leaf(&mut replay, &mut session, &target_payloads[0]);
            let second = session
                .allocate(&target_payloads[1], &[])
                .expect("allocate second leaf");
            replay
                .offer_replacement_leaf(&mut session, second)
                .expect("offer second leaf");
            assert_eq!(
                replay
                    .poll_replacement(&mut session)
                    .expect("partial carry"),
                PersistentSourceFactsHostReplayPoll::Pending
            );
            drop(replay);
            let build = session.suspend().expect("suspend partial replay");
            arena.abort_build(build).expect("abort partial replay");
        }
        drain_arena(&mut arena);
        assert_eq!(arena.metrics().resident_nodes, baseline.resident_nodes);
        assert_eq!(
