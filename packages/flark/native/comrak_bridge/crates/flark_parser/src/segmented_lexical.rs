...
//! it does not choose rule precedence. [`M11CleanBlockController`] owns those
//! grammar transitions; these states only summarize one forward-only physical
//! line or one leading reference-definition run. The same states are used for
//! every input size.
...
use crate::reference_label::{
    normalize_reference_label, reference_label_length_is_valid, MAX_RAW_REFERENCE_LABEL_UTF8_BYTES,
};
...
impl SegmentedLineScanner {
    ...
    #[must_use]
    pub(crate) const fn first_nonspace(&self) -> Option<usize> {
        self.first_nonspace
    }
...
pub(crate) struct SegmentedReferenceDefinition {
    pub(crate) source: Range<u32>,
    pub(crate) label_source: Range<u32>,
    pub(crate) destination_source: Range<u32>,
    pub(crate) title_source: Option<Range<u32>>,
    pub(crate) normalized_label: String,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum SegmentedReferenceTerminal {
    NoDefinitions {
        recognition_end: u32,
    },
    ReferenceOnly {
...
impl SegmentedReferenceTerminal {
    pub(crate) const fn prefix_end(self, base: u32) -> u32 {
        match self {
            Self::NoDefinitions { .. } => base,
            Self::ReferenceOnly { prefix_end } | Self::VisibleRemainder { prefix_end, .. } => {
                prefix_end
            }
        }
    }
...
#[derive(Clone, Debug)]
pub(crate) struct SegmentedReferencePrefix {
    base: u32,
    cursor: u32,
    prefix_end: u32,
    definition_count: usize,
    scanner: Option<ReferenceOne>,
    terminal: Option<SegmentedReferenceTerminal>,
}

impl SegmentedReferencePrefix {
    pub(crate) fn new(base: u32) -> Self {
        Self {
            ...
            prefix_end: base,
            definition_count: 0,
            scanner: Some(ReferenceOne::new(base)),
            terminal: None,
        }
    }

    pub(crate) fn awaiting_remainder(base: u32, definition_count: usize) -> Self {
        debug_assert!(definition_count > 0);
        Self {
            base,
            cursor: base,
            prefix_end: base,
    ...
    #[must_use]
    pub(crate) const fn terminal(&self) -> Option<SegmentedReferenceTerminal> {
        self.terminal
    }

    #[must_use]
    pub(crate) const fn definition_count(&self) -> usize {
    ...
    pub(crate) fn retained_source_bytes(&self) -> usize {
        self.scanner
            .as_ref()
            .map_or(0, |scanner| scanner.label.len())
    }
    ...
    pub(crate) fn push(
        ...
        absolute: u32,
        byte: u8,
    ) -> Result<Option<SegmentedReferenceDefinition>, SegmentedReferenceError> {
        if self.terminal.is_some() {
            return Ok(None);
        }
        if absolute != self.cursor {
            return Err(SegmentedReferenceError::NonSequential {
        ...
        let after = absolute
            .checked_add(1)
            .ok_or(SegmentedReferenceError::MetricOverflow)?;
        self.cursor = after;
        let scanner = self.scanner.take().expect("active reference scanner");
        let (scanner, decision) = scanner.consume(absolute, after, byte);
        self.apply_decision(scanner, decision, Some((absolute, after, byte)))
    }

    pub(crate) fn finish_eof(
        &mut self,
    ) -> Result<Vec<SegmentedReferenceDefinition>, SegmentedReferenceError> {
        let mut definitions = Vec::new();
        while self.terminal.is_none() {
            let scanner = self.scanner.take().expect("active reference scanner");
            let (scanner, decision) = scanner.finish_eof(self.cursor);
            if let Some(definition) = self.apply_decision(scanner, decision, None)? {
                definitions.push(definition);
            }
            // Acceptance at EOF rearms at the same cursor. The next loop
            // transition rejects the empty candidate and makes it terminal.
        }
        Ok(definitions)
    }
    ...
    fn apply_decision(
        &mut self,
        scanner: ReferenceOne,
        decision: ReferenceDecision,
        consumed: Option<(u32, u32, u8)>,
    ) -> Result<Option<SegmentedReferenceDefinition>, SegmentedReferenceError> {
        match decision {
            ReferenceDecision::Continue => {
                self.scanner = Some(scanner);
                Ok(None)
            }
            ReferenceDecision::Reject => {
                self.complete(self.cursor);
                Ok(None)
            }
            ReferenceDecision::Accept {
                ...
                let definition = scanner.build(source_end, retain_title)?;
                ...
                match consumed {
                    ...
                    Some((before, _, byte @ b'[')) if source_end == before => {
                        let replay = ReferenceOne::new(source_end);
                        let (replay, replay_decision) = replay.consume(before, before + 1, byte);
                        debug_assert!(matches!(replay_decision, ReferenceDecision::Continue));
                        self.scanner = Some(replay);
                    }
                    _ => self.complete(self.cursor),
                }
                Ok(Some(definition))
            }
            ReferenceDecision::Fallback { retain_title } => {
                if let Some(source_end) = scanner.fallback_source_end {
                    self.apply_decision(
                        scanner,
                        ReferenceDecision::Accept {
                            source_end,
    ...
    fn complete(&mut self, recognition_end: u32) {
        self.scanner = None;
        self.terminal = Some(if self.definition_count == 0 {
            SegmentedReferenceTerminal::NoDefinitions { recognition_end }
        } else if recognition_end == self.prefix_end {
            SegmentedReferenceTerminal::ReferenceOnly {
                prefix_end: self.prefix_end,
            }
        } else {
            SegmentedReferenceTerminal::VisibleRemainder {
                prefix_end: self.prefix_end,
                recognition_end,
            }
        });
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum SegmentedReferenceError {
    NonSequential { expected: u32, actual: u32 },
    InvalidUtf8,
    MetricOverflow,
}
...
// Orthogonal escape/newline flags make source replay unnecessary. Collapsing
// them into a product enum would obscure the donor transition correspondence.
#[allow(clippy::struct_excessive_bools)]
#[derive(Clone, Debug)]
struct ReferenceOne {
    start: u32,
    ...
    fallback_source_end: Option<u32>,
...
impl ReferenceOne {
    fn new(start: u32) -> Self {
        Self {
            start,
            phase: ReferencePhase::Label,
            label_start: start + 1,
            label: Vec::with_capacity(32),
    ...
    fn consume(mut self, before: u32, after: u32, byte: u8) -> (Self, ReferenceDecision) {
        let decision = match self.phase {
            ReferencePhase::Label => self.consume_label(before, byte),
            ReferencePhase::Colon => {
                if byte == b':' {
                    self.phase = ReferencePhase::BeforeDestination;
                    ReferenceDecision::Continue
                } else {
                    ReferenceDecision::Reject
                }
            }
            ReferencePhase::BeforeDestination => {
                self.consume_before_destination(before, after, byte)
            }
            ReferencePhase::AngleDestination => self.consume_angle_destination(before, byte),
            ReferencePhase::AngleClosed => {
                self.phase = ReferencePhase::AfterDestination;
                self.consume_after_destination(before, after, byte)
            }
            ReferencePhase::BareDestination => self.consume_bare_destination(before, after, byte),
            ReferencePhase::AfterDestination => self.consume_after_destination(before, after, byte),
            ReferencePhase::QuotedTitle => self.consume_quoted_title(after, byte),
            ReferencePhase::ParenTitle => self.consume_paren_title(after, byte),
            ReferencePhase::AfterTitle => self.consume_after_title(after, byte),
            ReferencePhase::AfterTitleCr => ReferenceDecision::Accept {
                source_end: if byte == b'\n' { after } else { before },
                retain_title: true,
            },
        };
        (self, decision)
    ...
    fn finish_eof(mut self, end: u32) -> (Self, ReferenceDecision) {
        let decision = match self.phase {
            ReferencePhase::BareDestination
                if self.destination_bytes > 0 && self.bare_depth == 0 =>
            {
                self.destination_end = end;
                ReferenceDecision::Accept {
                    source_end: end,
                    retain_title: true,
                }
            }
            ReferencePhase::AfterDestination
            | ReferencePhase::AfterTitle
            | ReferencePhase::AfterTitleCr => ReferenceDecision::Accept {
                source_end: end,
                retain_title: true,
            },
            ReferencePhase::QuotedTitle | ReferencePhase::ParenTitle => {
                ReferenceDecision::Fallback {
                    retain_title: false,
                }
            }
            _ => ReferenceDecision::Reject,
        };
        (self, decision)
    }

    fn consume_label(&mut self, before: u32, byte: u8) -> ReferenceDecision {
        ...
        if self.label_backslash {
            ...
            if is_ascii_punctuation(byte) {
                return self.append_label(byte);
            }
        }
        match byte {
            b']' => {
                if trimmed_label_offsets(&self.label).is_none() {
                    ReferenceDecision::Reject
                } else {
                    self.phase = ReferencePhase::Colon;
                    ReferenceDecision::Continue
                }
            }
            b'[' => ReferenceDecision::Reject,
            b'\\' => {
                self.label_backslash = true;
                self.append_label(byte)
            }
            _ => self.append_label(byte),
        }
    }

    fn append_label(&mut self, byte: u8) -> ReferenceDecision {
        self.label.push(byte);
        if self.label.len() > MAX_RAW_REFERENCE_LABEL_UTF8_BYTES {
            ReferenceDecision::Reject
        } else {
            ReferenceDecision::Continue
        }
    ...
    fn consume_before_destination(
        ...
        if self.before_destination_cr {
            ...
            if byte == b'\n' {
                return ReferenceDecision::Continue;
            }
            // A lone CR is a complete line ending. Reprocess the following
            // byte so post-ending spaces retain `spnl` semantics.
            return self.consume_before_destination(before, after, byte);
        }
        if matches!(byte, b' ' | b'\t') {
            return ReferenceDecision::Continue;
        }
        ...
        self.start_destination(before, after, byte)
    }

    fn start_destination(&mut self, before: u32, after: u32, byte: u8) -> ReferenceDecision {
        self.destination_start = before;
        if byte == b'<' {
            self.destination_start = after;
            self.phase = ReferencePhase::AngleDestination;
            ReferenceDecision::Continue
        } else {
            self.phase = ReferencePhase::BareDestination;
            self.consume_bare_destination(before, after, byte)
        }
    }

    fn consume_angle_destination(&mut self, before: u32, byte: u8) -> ReferenceDecision {
        ...
        match byte {
            ...
            b'>' => {
                self.destination_end = before;
                self.phase = ReferencePhase::AngleClosed;
            }
            b'\r' | b'\n' | b'<' => return ReferenceDecision::Reject,
            _ => {}
        }
        ReferenceDecision::Continue
    }

    fn consume_bare_destination(&mut self, before: u32, after: u32, byte: u8) -> ReferenceDecision {
        ...
        match byte {
            ...
            b'(' => {
                ...
                if self.bare_depth > 32 {
                    return ReferenceDecision::Reject;
                }
            }
            b')' => {
                if self.bare_depth == 0 {
                    return self.end_bare_destination(before, after, byte);
                }
                self.destination_bytes += 1;
                self.bare_depth -= 1;
            }
    ...
    fn end_bare_destination(
        ...
        self.destination_end = before;
        self.phase = ReferencePhase::AfterDestination;
        self.consume_after_destination(before, after, delimiter)
    }
    ...
    fn consume_after_destination(
        ...
        if self.after_destination_cr {
            ...
            if byte == b'\n' {
                self.fallback_source_end = Some(after);
                return ReferenceDecision::Continue;
            }
            self.fallback_source_end = Some(before);
            return self.consume_after_destination(before, after, byte);
        }
        if matches!(byte, b' ' | b'\t') {
        ...
        match byte {
            b'"' | b'\'' => {
                self.title_start = Some(before);
                self.title_closer = byte;
                self.phase = ReferencePhase::QuotedTitle;
                ReferenceDecision::Continue
            }
            b'(' => {
                self.title_start = Some(before);
                self.title_closer = b')';
                self.phase = ReferencePhase::ParenTitle;
                ReferenceDecision::Continue
            }
            _ => ReferenceDecision::Fallback {
                retain_title: false,
            },
        }
    }

    fn consume_quoted_title(&mut self, after: u32, byte: u8) -> ReferenceDecision {
        if self.consume_title_escape(byte) {
            return ReferenceDecision::Continue;
        }
        if byte == self.title_closer {
            self.title_end = Some(after);
            self.phase = ReferencePhase::AfterTitle;
        }
        ReferenceDecision::Continue
    }

    fn consume_paren_title(&mut self, after: u32, byte: u8) -> ReferenceDecision {
        if self.consume_title_escape(byte) {
            return ReferenceDecision::Continue;
        }
        match byte {
            ...
            b'(' => ReferenceDecision::Fallback {
                retain_title: false,
            },
            _ => ReferenceDecision::Continue,
        }
    }
    ...
    fn consume_after_title(&mut self, after: u32, byte: u8) -> ReferenceDecision {
        if matches!(byte, b' ' | b'\t') {
            return ReferenceDecision::Continue;
        }
        match byte {
            ...
            b'\r' => {
                self.phase = ReferencePhase::AfterTitleCr;
                ReferenceDecision::Continue
            }
            _ => ReferenceDecision::Fallback { retain_title: true },
        }
    }
    ...
    fn build(
        ...
        source_end: u32,
        retain_title: bool,
    ) -> Result<SegmentedReferenceDefinition, SegmentedReferenceError> {
        let label =
            std::str::from_utf8(&self.label).map_err(|_| SegmentedReferenceError::InvalidUtf8)?;
        if !reference_label_length_is_valid(label) {
            return Err(SegmentedReferenceError::InvalidUtf8);
        }
        let (trim_start, trim_end) =
            trimmed_label_offsets(&self.label).ok_or(SegmentedReferenceError::InvalidUtf8)?;
        let trim_start =
            u32::try_from(trim_start).map_err(|_| SegmentedReferenceError::MetricOverflow)?;
        let trim_end =
            u32::try_from(trim_end).map_err(|_| SegmentedReferenceError::MetricOverflow)?;
        let label_source = self.label_start + trim_start..self.label_start + trim_end;
        let normalized_label = normalize_reference_label(label);
        if normalized_label.is_empty() {
            return Err(SegmentedReferenceError::InvalidUtf8);
        }
        Ok(SegmentedReferenceDefinition {
            source: self.start..source_end,
            label_source,
            destination_source: self.destination_start..self.destination_end,
            title_source: retain_title
                .then(|| self.title_start.zip(self.title_end))
                .flatten()
...
enum ReferenceDecision {
    Continue,
    Reject,
    Accept { source_end: u32, retain_title: bool },
    Fallback { retain_title: bool },
}

fn trimmed_label_offsets(label: &[u8]) -> Option<(usize, usize)> {
    let mut start = 0;
    let mut end = label.len();
    while start < end && is_cmark_space(label[start]) {
        start += 1;
    }
    while end > start && is_cmark_space(label[end - 1]) {
        end -= 1;
    }
...
mod tests {
    ...
    fn scan_references(input: &str) -> Vec<SegmentedReferenceDefinition> {
        let mut scanner = SegmentedReferencePrefix::new(0);
        let mut definitions = Vec::new();
        for (offset, byte) in input.bytes().enumerate() {
            if let Some(definition) = scanner
                .push(u32::try_from(offset).unwrap(), byte)
                .expect("valid source")
            {
                definitions.push(definition);
            }
        }
        definitions.extend(scanner.finish_eof().expect("valid EOF"));
        definitions
    }

    fn compare_references(input: &str) {
        let actual = scan_references(input);
        let expected = block_spine_facade::reference_definitions(input).unwrap();
        assert_eq!(actual.len(), expected.len(), "presence {input:?}");
        for (actual, expected) in actual.iter().zip(expected) {
            assert_eq!(
                actual.source.start as usize..actual.source.end as usize,
                expected.source
            );
            assert_eq!(
                actual.label_source.start as usize..actual.label_source.end as usize,
                expected.label_source
            );
            assert_eq!(
                actual.destination_source.start as usize..actual.destination_source.end as usize,
                expected.url_source
            );
            assert_eq!(
                actual
    ...
    #[test]
    fn segmented_reference_dfa_matches_the_pinned_facade() {
        for fixed in [
            "[x]: /url\n",
            "[ x ]:\t<url> \"title\"\r\n",
            "[x]: a(b)c 'title'\n",
            ...
            compare_references(fixed);
        }

        let alphabet = b"[]:<>/()\\'\" abcXYZ09\t\r\n";
        let mut state = 0x0def_1a17_u64;
        for _ in 0..10_000 {
            state = state
                .wrapping_mul(6_364_136_223_846_793_005)
                .wrapping_add(1);
            let len = 1 + usize::try_from(state % 128).unwrap();
