// Build hook for the flark_tree_sitter code asset.
//
...
//   2. The consumer's pubspec user-define `hooks: user_defines: flark_tree_sitter:
//      prebuilt_dir: <dir>` holding <dir>/<triple>/<library>.
//   3. The co-located crate at this package's native/ built with cargo on
...
const _crateRelativePath = 'native';
...
void main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    if (plan == null) {
      throw BuildError(
        message:
            'flark_tree_sitter: unsupported target ${code.targetOS}/${code.targetArchitecture}',
      );
    }
    ...
    if (bundled.existsSync()) {
      ...
      if (!source.existsSync()) {
        throw BuildError(
          message:
              'flark_tree_sitter: the prebuilt_dir user-define is set but ${source.path} is missing',
        );
      }
      output.dependencies.add(source.uri);
      artifact = source.uri;
    } else if (Directory.fromUri(crateRoot).existsSync()) {
      for (final folder in ['src/', 'queries/']) {
        for (final f in Directory.fromUri(
          crateRoot.resolve(folder),
      ...
      throw BuildError(
        message:
            'flark_tree_sitter: no bundled prebuilt, no `prebuilt_dir` user-define, and no crate at ${crateRoot.toFilePath()}. Add to your pubspec:\n  hooks:\n    user_defines:\n      flark_tree_sitter:\n        prebuilt_dir: <directory holding ${plan.triple}/${plan.libraryFileName}>',
      );
    }
...
Future<Uri> _cargoBuild(_Plan plan, Uri crateRoot, Uri outputDir) async {
  ...
  if (rustup != null) {
    ...
    if (active.exitCode != 0 || toolchain.isEmpty) {
      throw BuildError(
        message:
            'flark_tree_sitter: rustup could not resolve a toolchain for ${crateRoot.toFilePath()}: ${active.stderr}',
      );
    }
    ...
    if (!installed.split('\n').map((l) => l.trim()).contains(plan.triple)) {
      ...
      if (add.exitCode != 0) {
        throw BuildError(
          message:
              'flark_tree_sitter: `rustup target add ${plan.triple} --toolchain $toolchain` failed:\n${add.stderr}',
        );
      }
    }
    ...
    if (cargo == null) {
      throw BuildError(
        message:
            'flark_tree_sitter: neither rustup nor cargo found. Install Rust (rustup.rs) or point the `prebuilt_dir` user-define at prebuilt libraries.',
      );
    }
    command = [cargo];
  }
  ...
  if (result.exitCode != 0) {
    throw BuildError(
      message:
          'flark_tree_sitter: cargo build failed\n${result.stdout}\n${result.stderr}',
    );
  }
  final built = File(
    targetDir.uri
  ...
  if (!built.existsSync()) {
    throw BuildError(
      message: 'flark_tree_sitter: expected artifact missing: ${built.path}',
    );
  }
  return built.uri;
}
...
class _Plan {
  ...
  static _Plan? resolve(CodeConfig code) {
    ...
    if (os == OS.linux) {
      ...
      return t == null
          ? null
          : _Plan(t, 'libflark_tree_sitter.so', () => const {});
    }
    if (os == OS.iOS) {
...
String _xcrun(String sdk, List<String> arguments) {
  ...
  throw BuildError(
    message:
        'flark_tree_sitter: xcrun could not resolve the $sdk toolchain: ${result.stderr}',
  );
}
...
Map<String, String> _androidEnvironment(
  ...
  if (ndk == null) {
    throw BuildError(
      message:
          'flark_tree_sitter: an Android build needs ANDROID_NDK_HOME, ANDROID_NDK, ANDROID_NDK_ROOT, ANDROID_NDK_LATEST_HOME, or ANDROID_HOME with an installed NDK.',
    );
  }
  final hostTag =
  ...
  if (hostTag == null) {
    throw BuildError(
      message:
          'flark_tree_sitter: no prebuilt LLVM toolchain under ${ndk.path}/toolchains/llvm/prebuilt.',
    );
  }
